...
def predict_and_visualize(image: Image.Image) -> tuple[np.ndarray, str]:
    ...
    """
    if image is None:
        # Return a blank image and message when no image is provided
        blank = np.zeros((224, 224, 3), dtype=np.uint8)
        return blank, "Please upload an image to analyze."
    ...
    panel_en = _add_label(overlay_en, f"CNN {label_en} ({conf_en:.1f}%)")
    ...
    panel_fv = _add_label(overlay_fv, f"ViT {label_fv} ({conf_fv:.1f}%)")
    ...
    summary = (
        f"CNN Model: {label_en} ({conf_en:.2f}% confidence)\n"
        f"ViT Model: {label_fv} ({conf_fv:.2f}% confidence)\n"
        f"CNN+ViT Model: {label_ef} ({conf_ef:.2f}% confidence)\n"
